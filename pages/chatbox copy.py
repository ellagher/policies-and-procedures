    ...
    return html.Div(
        [
            ...
            dbc.Spinner(
                ...
                color="primary",
                spinner_style={"display": "none", "margin": "10px auto"},  # Use spinner_style instead of style
            ),
            dcc.Store(id="session-id", storage_type="session"),
        ],
        style={
...
@callback(
    Output("chat-window", "children"),
    Output("chat-input", "value"),  
    Output("loading-indicator", "style"),  # Output for loading indicator
    Input("send-button", "n_clicks"),
    Input("chat-input", "n_submit"),  
    State("chat-input", "value"),
    State("chat-window", "children"),
...
def update_chat_window(n_clicks, n_submit, user_message, chat_history, session_id):
    ...
    loading_style = {"display": "block"}  # Make the loading spinner visible
